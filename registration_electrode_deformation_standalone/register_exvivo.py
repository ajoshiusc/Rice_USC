...
def run_registration(fixed_image_path, moving_atlas_path, atlas_labels_path, output_dir):
    ...
    # affine_transform_path = basename + ".affine.tfm" # Changed to .h5 below
    ...
    # We warp the atlas mask using the Rigid transform to get a rough brain mask in fixed space
    ...
    # Note: SimpleITK CompositeTransform applies transforms in the order they are added.
    # We want T_total(x) = T_rigid(T_affine(x))
    # So we add T_affine first, then T_rigid.
    ...
    # Flatten to avoid nested composite transforms which cause IO errors
    total_transform.FlattenTransform()
    
    # Save as HDF5 to support CompositeTransform properly
    ...
    warper.nonlinear_reg(
        ...
        # Robust Parameters for Ex Vivo / Damaged Tissue
        reg_penalty=2.0,      # Higher regularization
        nn_input_size=96,     # Higher resolution
        lr=5e-5,              # Lower learning rate
        max_epochs=7500,      # More epochs
        loss="cc"             # Cross-correlation
    )
...
if __name__ == "__main__":
    ...
    run_registration(args.fixed, args.atlas, args.labels, args.output)
